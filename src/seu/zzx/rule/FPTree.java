...
public class FPTree {
    ...
    public static void getRule(int sup,double confidence) throws IOException {
        String infile = "trolley.txt";
        FPTree fpTree = new FPTree();
        fpTree.setConfident(confidence);
        fpTree.setMinSuport(sup);
        List<List<String>> trans = fpTree.readTransRocords(new String[] { infile });
        Set<String> decideAttr = new HashSet<String>();
        decideAttr.add(ConstantLibrary.lostedFlagValue+"#"+ConstantLibrary.ISLOSTEDFLAGINDEX);
        decideAttr.add(ConstantLibrary.unlostedFlagValue+"#"+ConstantLibrary.ISLOSTEDFLAGINDEX);
        ...
        ExportRuleToFile.exportRuleToExcel(rules);
        
        ExportRuleToFile.exportRuleToText(ConstantLibrary.associationFilePath02, ConstantLibrary.associationFilePath04);
        ExportRuleToFile.exportRuleToText(ConstantLibrary.associationFilePath03, ConstantLibrary.associationFilePath05);
        
        ImportRuleToDB.createTable(ConstantLibrary.lostedRuleTableName);
        ImportRuleToDB.importDataToDB(ConstantLibrary.lostedRuleTableName, ImportRuleToDB.getRuleFromExcel(ConstantLibrary.associationFilePath02));
        ImportRuleToDB.createTable(ConstantLibrary.unlostedRuleTableName);
        ImportRuleToDB.importDataToDB(ConstantLibrary.unlostedRuleTableName, ImportRuleToDB.getRuleFromExcel(ConstantLibrary.associationFilePath03));
